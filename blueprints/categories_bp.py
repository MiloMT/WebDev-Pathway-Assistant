...
def create_card():
    ...
    category_info = CategorySchema(exclude=["id"]).load(request.json) # type: ignore
    
    category = Category(
        name = category_info["name"], # type: ignore
        description = category_info.get("description", "") # type: ignore
    )
...
def update_category(id):
    ...
    if category:
        ...
        category.name = category_info.get("name", category.name) # type: ignore
        category.description = category_info.get("description", category.description) # type: ignore
